...
def drive(vel_pub, twist, how_long):
    """
    Re-publish the provided twist for the indicated duration at a rate
    of 10hz.
    """
    rate = rospy.Rate(10)
    start_time = time.time()
    while time.time() < start_time + how_long and not rospy.is_shutdown():
...
def start():
    ...
    while not rospy.is_shutdown():
        ...
        if math.isnan(SCAN.ranges[320]) or SCAN.ranges[320] < target:
            twist.linear.x = -.2
            twist.angular.z = 0
            drive(vel_pub, twist, 1.0)
            twist.linear.x = 0
            twist.angular.z = -1.0
            drive(vel_pub, twist, 1.0)
        else:
            twist.linear.x = .1  # forward velocity in meters/second
            twist.angular.z = 0   # rotation in radians/second
        rospy.loginfo("Range: {}".format(SCAN.ranges[320]))
